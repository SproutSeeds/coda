...
export async function ensureDevModeSchema() {
  ...
  `);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS "idx_dev_usage_sessions_user" ON "dev_usage_sessions" ("user_id", "created_at");`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS "idx_dev_usage_sessions_idea" ON "dev_usage_sessions" ("idea_id", "created_at");`);
