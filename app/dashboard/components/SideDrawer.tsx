...
export function SideDrawer({
  ...
  return (
    <>
      {/* Tab Strip - Always Visible, offset from top for header */}
      <div
        className={cn(
          "fixed bottom-0 z-50 flex flex-col items-center py-6 gap-2",
          isLeft ? "left-0" : "right-0"
        )}
        style={{ width: 56, top: 80 }}
      >
        {tabs.map((tab) => {
      ...
      <AnimatePresence>
        {activeTab && activeTabData && (
          <motion.div
            ...
            className={cn(
              "fixed bottom-0 z-40 overflow-hidden",
              isLeft ? "left-14" : "right-14"
            )}
            style={{ width: panelWidth, top: 80 }}
